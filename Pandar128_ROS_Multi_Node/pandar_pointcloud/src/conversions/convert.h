...
#define PANDAR128_HEAD_SIZE           (PANDAR128_SOB_SIZE + \
    PANDAR128_VERSION_MAJOR_SIZE + PANDAR128_VERSION_MINOR_SIZE + \
    PANDAR128_HEAD_RESERVED1_SIZE + PANDAR128_LASER_NUM_SIZE + \
    PANDAR128_BLOCK_NUM_SIZE + PANDAR128_ECHO_COUNT_SIZE + \
    PANDAR128_ECHO_NUM_SIZE + PANDAR128_HEAD_RESERVED2_SIZE)
#define PANDAR128_AZIMUTH_SIZE        (2)
#define DISTANCE_SIZE                 (2)
#define INTENSITY_SIZE                (1)
#define CONFIDENCE_SIZE               (1)
#define PANDAR128_UNIT_SIZE           (DISTANCE_SIZE + \
    INTENSITY_SIZE + CONFIDENCE_SIZE)
#define PANDAR128_BLOCK_SIZE          (PANDAR128_UNIT_SIZE * \
    PANDAR128_LASER_NUM + PANDAR128_AZIMUTH_SIZE)
...
#define PANDAR128_TAIL_SIZE           (PANDAR128_TAIL_RESERVED1_SIZE + \
    PANDAR128_TAIL_RESERVED2_SIZE + PANDAR128_SHUTDOWN_FLAG_SIZE + \
    PANDAR128_TAIL_RESERVED3_SIZE + PANDAR128_MOTOR_SPEED_SIZE + \
    PANDAR128_TS_SIZE + PANDAR128_RETURN_MODE_SIZE + \
    PANDAR128_FACTORY_INFO + PANDAR128_UTC_SIZE)
#define PANDAR128_PACKET_SIZE         (PANDAR128_HEAD_SIZE + \
    PANDAR128_BLOCK_SIZE * PANDAR128_BLOCK_NUM + PANDAR128_TAIL_SIZE)
#define PANDAR128_SEQ_NUM_SIZE        (4)
#define PANDAR128_PACKET_SEQ_NUM_SIZE (PANDAR128_PACKET_SIZE + \
...
typedef struct PacketsBuffer_s {
  ...
  inline bool hasEnoughPackets() {
    return ((m_iterPush > m_iterTaskBegin && m_iterPush > m_iterTaskEnd) || (m_iterPush < m_iterTaskBegin && m_iterPush < m_iterTaskEnd));
  }
...
public:
    ...
    Convert(ros::NodeHandle node, ros::NodeHandle private_nh, std::string nod_typee=LIDAR_NODE_TYPE);
    ~Convert() {}
